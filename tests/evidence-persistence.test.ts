...
import { KNOWN_SKILL, serviceWith } from "./support/evidence-fixtures.ts";
...
test("PostgreSQL interview answers survive application restart and stay atomic", {
  ...
  const makeApp = () => new EvidenceApplication(new PostgresEvidenceRepository(pool), serviceWith());
  try {
    await ensureSchema(pool);
    const first = makeApp();
    const started = await first.startInterview(context(), { target: { kind: "skills", skillIds: [KNOWN_SKILL] }, questionCount: 3 });
    ...
    const submitted = await first.submitInterviewAnswer(context(),
      { interviewId: interview.interviewId, questionId: interview.questions[0].questionId, answer: answerText });
    ...
    const feedback = await restarted.getInterviewFeedback(context(), {
      interviewId: interview.interviewId, questionId: interview.questions[0].questionId,
    });
    assert.ok(feedback.ok && feedback.data && "feedback" in feedback.data);
    assert.equal(feedback.data.feedback?.status, "succeeded");
    assert.ok((feedback.data.feedback?.suggestions.length ?? 0) > 0);
    ...
    const duplicate = await restarted.submitInterviewAnswer(context(),
      { interviewId: interview.interviewId, questionId: interview.questions[0].questionId, answer: answerText });
    ...
    invalid.answers.push({
      answerId: randomUUID(), questionId: randomUUID(), text: "invalid reference",
      feedback: null, createdAt: new Date().toISOString(),
    });
    ...
    const next = await makeApp().submitInterviewAnswer(context(),
      { interviewId: interview.interviewId, questionId: interview.questions[1].questionId, answer: "I compare query plans before and after the change." });
    assert.equal(next.ok, true);
    const finished = await makeApp().finishInterview(context(), { interviewId: interview.interviewId });
    ...
    assert.equal(final.data.page.items[0].answeredCount, 2);
    assert.equal(final.data.session, null);
    const hidden = await makeApp().getInterviewFeedback({ ownerId: `${owner}-other` }, { interviewId: interview.interviewId });
