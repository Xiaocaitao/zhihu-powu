...
test("四个成长空间业务块注册到统一 Agent Registry", () => {
  ...
  for (const name of ["get_user_profile", "get_career_plan", "get_active_learning_plan", "get_learning_feedback", "get_learning_records", "start_interview"]) assert.ok(names.includes(name), name);
