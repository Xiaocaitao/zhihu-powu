...
export function createLearningCapabilities(service: LearningApplication): DomainCapability[] { return [
  ...
  { name: "get_learning_feedback", description: "读取当前用户学习计划的反馈记录。", inputSchema: { type: "object", properties: { planId: { type: "string" } }, additionalProperties: false }, execute: async (ctx, input) => read(await service.getLearningFeedback(ctx, (input ?? {}) as { planId?: string }), "已读取学习计划反馈") },
