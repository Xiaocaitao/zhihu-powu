...
export function createCareerCapabilities(service: CareerApplication): DomainCapability[] {
  return [
    { name: "get_career_plan", description: "读取当前用户职业规划。", inputSchema: getCareerPlanSchema, execute: (ctx, input) => service.getCareerPlan(ctx, getCareerPlanSchema.parse(input)) as Promise<any> },
    ...
    { name: "get_target_jobs", description: "读取当前用户保存的目标岗位。", inputSchema: getTargetJobsSchema, execute: (ctx, input) => service.getTargetJobs(ctx, getTargetJobsSchema.parse(input)) as Promise<any> },
    ...
    { name: "get_career_dashboard", description: "读取职业规划页所需的聚合上下文。", inputSchema: { type: "object", additionalProperties: false }, execute: ctx => service.getCareerDashboard(ctx) as Promise<any> },
