...
export function createCareerCapabilities(service: CareerApplication): DomainCapability[] {
  return [
    ...
    { name: "get_career_dashboard", description: "读取职业规划页所需的聚合上下文。", inputSchema: getCareerDashboardSchema, execute: async (ctx, input) => read(await service.getCareerDashboard(ctx, getCareerDashboardSchema.parse(input)), "已读取职业规划聚合上下文") },
    ...
    { name: "list_target_companies", description: "读取当前用户可访问的企业及其关联岗位。", inputSchema: listTargetCompaniesSchema, execute: async (ctx, input) => read(await service.listTargetCompanies(ctx, listTargetCompaniesSchema.parse(input)), "已读取目标企业") },
    { name: "select_target_company", description: "在用户明确选择后设置当前目标企业。", inputSchema: selectTargetCompanySchema, execute: (ctx, input) => service.selectTargetCompany(command(ctx, selectTargetCompanySchema.parse(input))), requiresConfirmation: true },
    { name: "compare_target_companies", description: "比较两个当前用户可访问的企业及其关联岗位。", inputSchema: compareTargetCompaniesSchema, execute: (ctx, input) => service.compareTargetCompanies(ctx, compareTargetCompaniesSchema.parse(input)) },
    { name: "get_industry_trends", description: "读取职业方向的行业趋势数据。", inputSchema: getIndustryTrendsSchema, execute: async (ctx, input) => read(await service.getIndustryTrends(ctx, getIndustryTrendsSchema.parse(input)), "已读取行业趋势") },
  ];
}
