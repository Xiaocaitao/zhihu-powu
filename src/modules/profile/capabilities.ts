...
import type { SaveProfileFactInput } from './contracts.ts';
...
export function createProfileCapabilities(service:ProfileApplication):DomainCapability[]{
 ...
 return [
  ...
  {name:'update_user_goal',description:'更新用户目标方向',inputSchema:updateUserGoalInputSchema,execute:async(ctx,input)=>service.updateUserGoal(command(ctx,updateUserGoalInputSchema.parse(input)))},
 ];
}
