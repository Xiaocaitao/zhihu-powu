...
export class MemoryProfileRepository implements ProfileRepository {
 ...
 async withWriteTransaction<T>(fn:ProfileWriteTransaction<T>):Promise<T>{const facts=new Map([...this.facts].map(([o,m])=>[o,new Map(m)]));const goals=new Map(this.goals);const receipts=new Map([...this.receipts].map(([o,m])=>[o,new Map(m)]));try{return await fn(this)}catch(e){this.facts=facts;this.goals=goals;this.receipts=receipts;throw e;}}
}
