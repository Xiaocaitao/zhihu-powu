...
export type PromptContextInput = { message: string; sessionId: string; ownerId?: string; requestId?: string };
...
export async function buildPrompt(input: PromptContextInput, context?: PromptContext) {
  return [personality, toolBoundary, growthWorkflow, context ? await context(input) : ""].filter(Boolean).join("\n\n");
}
