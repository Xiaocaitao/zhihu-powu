...
export function createDoubaoModels(modelId: string, baseUrl = "https://ark.cn-beijing.volces.com/api/v3"): MutableModels {
  const model: Model<"openai-responses"> = { id: modelId, name: `Doubao ${modelId}`, api: "openai-responses", provider: "doubao", baseUrl, reasoning: true, input: ["text"], cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }, contextWindow: 128000, maxTokens: 8192, compat: { supportsDeveloperRole: false, supportsLongCacheRetention: false, supportsStrictMode: false } };
